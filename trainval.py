...
class Trainval(object):
    def __init__(self, opt):
        ...
        # Always profile model once at startup for parameter/FLOPs comparison
        self.model_profile_mac_params()

    def model_profile_mac_params(self):
        device = torch.device(f"cuda:{self.opt.gpu_ids[0]}") if torch.cuda.is_available() and len(self.opt.gpu_ids) > 0 else torch.device("cpu")
        H = self.opt.input_size
        W = self.opt.input_size
        pre_img = torch.randn(1, 3, H, W, device=device)
        post_img = torch.randn(1, 3, H, W, device=device)
        mac, prarms = profile(self.model.detector, (pre_img, post_img))
        print(f"FLOPs (detector): {mac*2 / 1e9:.12f} G")    
        print(f"Number of parameters (detector, thop): {prarms / 1e6:.12f} M")
        total_params = sum(p.numel() for p in self.model.detector.parameters())
        print(f"Params (detector only): {total_params / 1e6:.12f} M")
...
if __name__ == "__main__":
    ...
    if getattr(opt, 'deterministic', False):
        setup_seed(seed=getattr(opt, 'seed', 63))
    else:
